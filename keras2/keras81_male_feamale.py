...
img_human = load_img('C:/Study/image/human/taeuk1/taeuk2/final_1000.jpg', target_size=(224,224))
...
arr_human = img_to_array(img_human)
#형식 자체가 달라서 array로 바꾼다 => 수치화시킨다 
arr_cat = img_to_array(img_cat)
arr_lion = img_to_array(img_lion)
arr_suit = img_to_array(img_suit)
print(arr_human)
print(type(arr_human)) #class 'numpy.ndarray'
print(arr_human.shape) #(224, 224, 3)

#RGB -> BGR 바꿔줘야한다
from tensorflow.keras.applications.vgg16 import preprocess_input
#array 한다음 preprocess 해주면 알아서 vgg16에 맞춰서 변환 
arr_human = preprocess_input(arr_human)
arr_cat = preprocess_input(arr_cat)
arr_lion = preprocess_input(arr_lion)
arr_suit = preprocess_input(arr_suit)
print(arr_human) #위에랑 달라짐 
print(arr_human.shape) #(224, 224, 3)

# arr_input = np.stack([arr_human, arr_cat, arr_lion, arr_suit])
#stack - 순서대로 (224,244,3)이 합쳐짐
# print(arr_input.shape) #(4, 224, 224, 3)
arr_human = arr_human.reshape(1,224,224,3)

#2. 모델구성
model = VGG16()
results = model.predict(arr_human)
