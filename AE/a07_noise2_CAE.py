...
x_train_out = x_train.reshape(60000, 28,28,1)/255
x_test = x_test.reshape(10000, 784)/255. #위에꺼랑 같음

#  노이즈 임의로 만든다 
x_train_noised = x_train + np.random.normal(0, 0.1, size=x_train.shape)
x_train_noised_out = x_train_out + np.random.normal(0, 0.1, size=x_train.shape)
x_test_noised = x_test + np.random.normal(0, 0.1, size=x_test.shape)
x_train_noised = np.clip(x_train_noised, a_min=0, a_max=1) #고정시킨다. 1이상이 되면 1로 고정 시킨다 . (a_min은 상관없음)
x_test_noised = np.clip(x_test_noised, a_min=0, a_max=1)
x_train_noised_out = np.clip(x_train_noised_out, a_min=0, a_max=1)
...
model.fit(x_train_noised_out, x_train_out, epochs=10)
