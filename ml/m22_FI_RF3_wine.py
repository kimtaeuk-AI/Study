...
for i in range(len(fi)):
    if fi[i] != 0:
        ...
        feature.append(dataset.target[i])

new_data = np.array(new_data) #(13,178)
new_data = np.transpose(new_data)

x_train,x_test, y_train, y_test = train_test_split(new_data, dataset.target, train_size=0.8, shuffle=True, random_state=77)
...
def plot_feature_importances_dataset(model):
    ...
    plt.barh(np.arange(n_features), sorted(fi), align='center')
