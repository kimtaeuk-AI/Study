...
x_train, x_test, y_train, y_test = train_test_split(x, y, train_size=0.8, shuffle=True) # Shuffle = Flase 는 똑바로 
print(x_train)
print(x_train.shape)
print(x_test.shape)
print(y_train.shape)
...
model.fit(x_train, y_train, epochs=100, validation_split=0.2) #우리가 필요한 최적의 웨이트값을 구하기 위해 엑스트레인 와이트레인을 넣는다 
